...
from sglang.kernels.ops.memory.dsa_cp_shared import (
...
class MLATokenToKVPool(KVCache):
    ...
    def _create_cp_shared_buffers(self) -> None:
        ...
        self.cp_shared_cp_size = get_parallel().attn_cp_size
        assert self.cp_shared_cp_size > 1
        self.cp_shared_cp_rank = get_parallel().attn_cp_rank
    ...
    def translate_loc_to_cp_shared_device_int32(self, loc: torch.Tensor) -> torch.Tensor:
        if not self.enable_cp_shared_kvcache:
    ...
    def set_mla_kv_buffer(
        ...
        self._write_mla_kv_buffer(
            self._get_mla_write_buffer(layer_id),
            loc,
            cache_k_nope,
            cache_k_rope,
        )
